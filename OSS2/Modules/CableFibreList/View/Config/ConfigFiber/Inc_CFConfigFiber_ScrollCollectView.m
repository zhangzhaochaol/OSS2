//
//  Inc_CFConfigFiber_ScrollCollectView.m
//  OSS2.0-ios-v2
//
...
#import "Inc_CFConfigFiber_ScrollCollectView.h"

#import "Yuan_ScrollCollectionVM.h"         // 用来判断 行优列优等排序方式的算法

#import "Inc_CFScrollHorizontalCell.h"     // 行优 CollectionViewCell
#import "Inc_CFScrollVerticalCell.h"       // 列优 collectionViewCell
...
@interface Inc_CFConfigFiber_ScrollCollectView ()
...
@implementation Inc_CFConfigFiber_ScrollCollectView
...
//    NSArray * opticTermsArray = [value objectForKey:@"opticTerms"];
//
//    inLineNums = (int)opticTermsArray.count;
...
/// 行优
- (Inc_CFScrollHorizontalCell *)horizontalCellWithCollectionView:(UICollectionView *)collectionView
                                         cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    
    
    Inc_CFScrollHorizontalCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"Yuan_CFScrollHorizontalCell" forIndexPath:indexPath];
    
    if (!cell) {
        cell = [[Inc_CFScrollHorizontalCell alloc] init];
    }
...
/// 列优
- (Inc_CFScrollVerticalCell *)verticalCellWithCollectionView:(UICollectionView *)collectionView
                                            cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    
    
    Inc_CFScrollVerticalCell * cell =
    [collectionView dequeueReusableCellWithReuseIdentifier:@"Yuan_CFScrollVerticalCell"
                                              forIndexPath:indexPath];
    
    if (!cell) {
        cell = [[Inc_CFScrollVerticalCell alloc] init];
    }
...
- (void) collectionInit {
    ...
    if (_import == Important_Line) {
        ...
        // 单个 模块的宽度
//        float horTermolFixed = KItemFixed * (_inlineNums + 2);
    ...
    if (_import == Important_row) {
        ...
        // 单个 模块的高度 !!!  列优是高度
//        float verTermolFixed = KItemFixed * (_inlineNums + 2);
...
- (UICollectionView *) collection :(int) i {
   ...
   forCellWithReuseIdentifier:@"collection"];
    ...
    [collection registerClass:[Inc_CFScrollHorizontalCell class]
   forCellWithReuseIdentifier:@"Yuan_CFScrollHorizontalCell"];
    
    [collection registerClass:[Inc_CFScrollVerticalCell class]
   forCellWithReuseIdentifier:@"Yuan_CFScrollVerticalCell"];
...
/// MARK: 接下来的代码的唯一目的就是跳转 按钮添加进数组的顺序 , 因为在之前的逻辑中 , 一行collectionViewCell里有多个item , 也就是端子盘里的模块 (包含两个绑卡之间的端子总称) , 模块生成的顺序不是 1,2,3 这样的顺序生成 , 而是跳跃生成的 . 但此处的业务 , 需要将这些按钮 , 按绑卡顺序注入到数组当中 .  接下来 按 绑卡顺序 把按钮 按顺序 添加到 terminalBtnArray 数组中
    ...
    if (_import == Important_Line) {
        ...
        while (position <= _lineCount * _rowCount) {
            for (Inc_CFScrollHorizontalCell * item in _allCollectionItemArray) {
    ...
    if (_import == Important_row) {
        ...
        while (position < _lineCount * _rowCount) {
            for (Inc_CFScrollVerticalCell * item in _allCollectionItemArray) {
                if ([item.position_btnArray_Dict.allKeys.firstObject integerValue] == position) {
                    [_viewModel.terminalBtnArray addObjectsFromArray:item.position_btnArray_Dict.allValues.firstObject];
