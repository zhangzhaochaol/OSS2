//
//  Inc_WebViewJavascriptBridgeBase.m
//
...
#import "Inc_WebViewJavascriptBridge_JS.h"

@implementation Inc_WebViewJavascriptBridgeBase {
...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (Inc_WVJBMessage* message in messages) {
        if (![message isKindOfClass:[Inc_WVJBMessage class]]) {
        ...
        if (responseId) {
            Inc_WVJBResponseCallback responseCallback = _responseCallbacks[responseId];
            responseCallback(message[@"responseData"]);
            [self.responseCallbacks removeObjectForKey:responseId];
        } else {
            Inc_WVJBResponseCallback responseCallback = NULL;
            ...
            if (callbackId) {
                responseCallback = ^(id responseData) {
                    ...
                    Inc_WVJBMessage* msg = @{ @"responseId":callbackId, @"responseData":responseData };
            ...
            Inc_WVJBHandler handler = self.messageHandlers[message[@"handlerName"]];
...
- (void)injectJavascriptFile {
    NSString *js = Inc_WebViewJavascriptBridge_js();
...
- (void)_queueMessage:(Inc_WVJBMessage*)message {
    if (self.startupMessageQueue) {
...
- (void)_dispatchMessage:(Inc_WVJBMessage*)message {
