...
- (instancetype)initWiithMenuItemTitles:(nonnull NSArray *)titles ImageNames:(NSArray *)imageNames withMenuData:(MLMenuData *)menuData cellHeight:(CGFloat)cellHeight
{
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < self.subviews.count; i ++) {
        ...
        frame.origin.x = 0;
        frame.origin.y = (_cellHeight + 1) * i;
        frame.size = CGSizeMake(self.bounds.size.width, (_cellHeight + 1));
...
- (void)initialize:(NSInteger)index{
    ...
    if (self.imageName) {
        [button setImage:[UIImage Inc_imageNamed:self.imageName] forState:UIControlStateNormal];
    }
