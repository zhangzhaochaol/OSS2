...
- (instancetype)initWithFrame:(CGRect)frame
                   WithTitles:(nonnull NSArray *)titles
               WithImageNames:(nullable NSArray *)imageNames
        WithMenuViewOffsetTop:(CGFloat)top
       WithTriangleOffsetLeft:(CGFloat)left
                triangleColor:(nullable UIColor *)triangleColor
                   cellHeight:(float) cellHeight{
    
    self = [super initWithFrame:CGRectMake(0, 0, k_ScreenWidth, k_ScreenHeight)];
    if (self) {
        _frame = CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, titles.count * cellHeight);
       _menuViewOffsetTop = _menuViewOffsetTop < k_StatusBarAndNavigationBarHeight ? k_StatusBarAndNavigationBarHeight : _menuViewOffsetTop;
        _triangleOffsetLeft = left;
        _menuViewOffsetTop = top;
        _isHasTriangle = YES;
...
- (void)layoutSubviews{
    ...
    CGFloat itemHeight = _cellHeight + 1;
...
- (CGFloat)calculateMenuWidth{
    ...
    if (self.imageNames.count > 0) {
        for (NSString *imageName in self.imageNames) {
            UIImage *image = [UIImage Inc_imageNamed:imageName];
...
{
    if (!_itemsView) {
        _itemsView = [[MLMenuItemsView alloc] initWiithMenuItemTitles:self.menuData.titles ImageNames:self.menuData.imageNames withMenuData:self.menuData cellHeight:_cellHeight];
